...
Cuando `wrangler secret put OPENAI_API_KEY` lo solicite, pega tu clave de OpenAI.

Después, en **Japoteacher → Ajustes → Evaluación con IA**:

- Proveedor: `OpenAI`.
- La sesión de Supabase autoriza automáticamente el uso del Worker.

La configuración completa está en [worker/README.md](worker/README.md).

## Seguridad

No se incluyen claves privadas. `OPENAI_API_KEY` solo existe en Cloudflare. El Worker restringe orígenes y valida la sesión de Supabase antes de consumir créditos.
...
Después, crea una cuenta desde **Ajustes → Cuenta y sincronización** e inicia sesión con la misma cuenta en el PC y el móvil. No es necesario configurar URLs, claves ni tokens en los dispositivos.
