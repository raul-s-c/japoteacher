(function () {
  ...
  function experienceHtml(metrics) {
    ...
    const activityLabels={translation_attempt:'Traducciones',daily_news_answer:'Noticias'},activities=Object.entries(metrics.activity||{}).filter(([key,item])=>key!=='lexical_review'&&item.count).map(([key,item])=>`<article><span>${activityLabels[key]||key}</span><strong>${format(item.net)} EXP</strong><small>${item.count} acciones</small></article>`).join('');
