(function () {
  ...
  function experienceHtml(metrics) {
    if (!metrics?.attempt_count) return '<section class="report-experience report-experience-empty"><p>EXP ranked</p><span>La trazabilidad de EXP comenzará con las próximas respuestas.</span></section>';
    const format = value => `${Number(value) >= 0 ? "+" : ""}${new Intl.NumberFormat("es-ES", { maximumFractionDigits: 2 }).format(Number(value) || 0)}`;
    const directions = [["ja_es", "JP a ES"], ["es_ja", "ES a JP"]].map(([key, label]) => `<article><span>${label}</span><strong>${format(metrics.direction?.[key]?.net)} EXP</strong><small>${metrics.direction?.[key]?.count || 0} respuestas</small></article>`).join("");
    const activityLabels={translation_attempt:'Traducciones',daily_news_answer:'Noticias',lexical_review:'Micro-SRS'},activities=Object.entries(metrics.activity||{}).filter(([,item])=>item.count).map(([key,item])=>`<article><span>${activityLabels[key]||key}</span><strong>${format(item.net)} EXP</strong><small>${item.count} acciones</small></article>`).join('');
    const daily = (metrics.daily || []).map(day => `<li><span>${periodLabel(`${day.date}T12:00:00Z`)}</span><b>${format(day.net)} EXP</b><em>${day.count} respuestas</em></li>`).join("");
    return `<section class="report-experience"><div><p class="report-section-label">Evolución ranked</p><h5>EXP del periodo</h5><span class="report-experience-total">${format(metrics.net)} EXP neta</span><small>${format(metrics.gained)} ganada · ${format(metrics.lost)} perdida</small></div><div class="report-experience-directions">${directions}</div>${activities?`<div class="report-experience-directions report-experience-activities">${activities}</div>`:''}${daily ? `<ol class="report-experience-daily">${daily}</ol>` : ""}</section>`;
  }
  const baseReadyBody = readyBody;
  readyBody = report => `${baseReadyBody(report)}${experienceHtml(report.experience_metrics)}`;
