(function(){
  ...
  async function load(){
    ...
    try{
      const [raw,exercises,attempts,progress,settings]=await Promise.all([data||fetch('data/knowledge-map.json?v=20260914-editorial-247').then(r=>{if(!r.ok)throw Error('No se pudo descargar el mapa.');return r.json()}),JapoDB.all('exercises'),JapoDB.all('attempts'),JapoDB.all('exercise_progress'),JapoDB.get('settings','app')]);
