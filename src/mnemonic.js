(function(){
  ...
  function html(attemptId,errors,attempt){
    ...
    return `<section class="mnemonic-card" data-mnemonic-attempt="${esc(attemptId)}"><h4>¿Quieres un consejo mnemotécnico?</h4><p>${errors.length?'Un truco breve para recordar lo que has fallado.':'Un truco breve para recordar el vocabulario o la estructura de esta frase.'}</p><label>¿Qué palabra o expresión quieres recordar?<input type="text" data-mnemonic-target maxlength="100" placeholder="Ej.: 本 o libro" value="${esc(value?.selected_target||'')}"></label><p><small>En japonés o español. Déjalo vacío para elegir automáticamente.</small></p><button type="button" class="secondary" data-mnemonic-generate${value?' hidden':''}>Dame un consejo</button><div class="mnemonic-result" aria-live="polite"${value?'':' hidden'}>${value?content(value):''}</div><small>Las asociaciones son ayudas de memoria inventadas.</small></section>`;
  }
  async function generate(id,refresh=false,selectedTarget=''){
    ...
    const cached=saved(attempt);if(cached&&!refresh&&(cached.selected_target||'')===selectedTarget)return cached;
    ...
    data.mnemonic.selected_target=selectedTarget;
  ...
  async function click(button){
    ...
    const input=card.querySelector('[data-mnemonic-target]'),selectedTarget=input.value.trim();input.disabled=true;
    ...
    try{
      if(!pending.has(id))pending.set(id,generate(id,button.hasAttribute('data-mnemonic-refresh'),selectedTarget).finally(()=>pending.delete(id)));
    ...
    finally{input.disabled=false;button.disabled=false;if(refresh&&button.hidden)button.remove()}
  }
  ...
  document.addEventListener('input',event=>{if(!event.target.matches('[data-mnemonic-target]'))return;const card=event.target.closest('[data-mnemonic-attempt]'),button=card.querySelector('[data-mnemonic-generate]');button.hidden=false;button.textContent=event.target.value.trim()?'Crear truco para esta palabra':'Dame un consejo';card.querySelector('.mnemonic-result').hidden=true;});
