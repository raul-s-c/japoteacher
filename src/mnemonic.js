(function(){
  ...
  async function generate(id){
    ...
    const cached=saved(attempt);if(cached)return cached;
  ...
  async function click(button){
    ...
    button.disabled=true;button.textContent='Preparando consejo…';target.hidden=false;target.textContent='Buscando una asociación fácil de recordar…';
    try{
      if(!pending.has(id))pending.set(id,generate(id).finally(()=>pending.delete(id)));
  ...
  document.addEventListener('click',event=>{const button=event.target.closest('[data-mnemonic-generate]');if(button&&!button.disabled)void click(button)});
