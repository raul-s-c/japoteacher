(function(){
  ...
  function evidenceByLevel(attempts,exercises){
    ...
    return levels.map(level=>{
      ...
      return {level,count:rows.length,distinct:new Set(rows.map(row=>row.exercise_id)).size,average:average(scores),acceptableRate:rows.length?Math.round(acceptable*100/rows.length):0,difficulty:average(difficulty)};
    });
  }
  function assessment(rows){
    const requirements=window.RankedProgress?.masteryRequirements||{},practiced=rows.filter(row=>row.count),established=rows.filter(row=>row.distinct>=8),current=established.at(-1)||practiced.at(-1)||rows[0],requirement=requirements[current.level]||{distinct:12,average:70,acceptable:60},reliable=current.distinct>=Math.min(8,requirement.distinct),consolidated=reliable&&current.distinct>=requirement.distinct&&current.average>=requirement.average&&current.acceptableRate>=requirement.acceptable,next=levels[levels.indexOf(current.level)+1],gaps=[];
    if(current.distinct<requirement.distinct)gaps.push(`resolver ${requirement.distinct-current.distinct} frases distintas más`);
    if(current.count&&current.average<requirement.average)gaps.push(`elevar la media ${requirement.average-current.average} puntos`);
    if(current.count&&current.acceptableRate<requirement.acceptable)gaps.push(`mejorar respuestas aceptables ${requirement.acceptable-current.acceptableRate} puntos porcentuales`);
  ...
  function directionCard(direction,attempts,exercises){
    ...
    return `<article class="teacher-direction"><header><p>${esc(directionName(direction))}</p><strong>${esc(current.level)}</strong></header><div class="teacher-levels">${rows.map(row=>`<span class="${row.level===current.level?'active':''}"><b>${row.level}</b><em>${row.average==null?'—':`${row.average}%`}</em></span>`).join('')}</div><dl><div><dt>Estado</dt><dd>${state}</dd></div><div><dt>Evidencia</dt><dd>${current.distinct} frases distintas · ${current.count} respuestas · ${difficulty}</dd></div></dl><p class="teacher-next">${esc(nextText)}</p></article>`;
  }
  function focus(attempts,exercises){
    ...
    if(!weakest)return {title:'Construye la primera evidencia',copy:'Resuelve frases de varias familias. La app necesita diversidad, no repeticiones inmediatas, para situarte con rigor.',target:'Primera semana de evidencia'};
