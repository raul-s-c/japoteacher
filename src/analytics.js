(function(){
  ...
  async function snapshot(){
    ...
    const validAttempts=attempts.filter(attempt=>attempt.evaluation_status!=='invalid'&&Number.isFinite(Number(attempt.overall_score))&&eMap.get(attempt.exercise_id)?.active!==false);
    ...
    cache={attempts:validAttempts.sort((a,b)=>b.attempted_at.localeCompare(a.attempted_at)),allAttempts:attempts.sort((a,b)=>b.attempted_at.localeCompare(a.attempted_at)),validAttempts,progress,exercises,eMap,dirs,tags,tagGroups:groupTags(tags),completedDays};
    return cache;
  }
