(function(){
  ...
  async function snapshot(){
    ...
    const persistedNewsIds=new Set(attempts.filter(attempt=>attempt.study_event_type==='daily_news_answer').map(attempt=>attempt.attempt_id));
    const newsStudyAttempts=newsAnswers.filter(answer=>Number.isFinite(Number(answer.overall_score))&&!persistedNewsIds.has(answer.answer_id)).map(answer=>({
      ...answer,
