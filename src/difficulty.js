(function(){
  const legacyRanges={N5:[1,4],N4:[4,7],N3:[1,7],N2:[1,7],N1:[1,7]},levels=['N5','N4','N3','N2','N1'],bands=[{min:0,max:24},{min:25,max:49},{min:50,max:74},{min:75,max:100}];
  function score(exercise){const raw=Number(exercise?.difficulty),version=Number(exercise?.dataset_version);if(!Number.isFinite(raw))return 50;if(version>=4||raw===0||raw>7)return Math.round(Math.max(0,Math.min(100,raw)));const [min,max]=legacyRanges[exercise?.jlpt_level]||[1,7];return Math.round(100*Math.max(0,Math.min(1,(raw-min)/Math.max(1,max-min))))}
  ...
  function band(value){return bands.findIndex(range=>value>=range.min&&value<=range.max)}
  function bandFor(exercise){return band(score(exercise))}
  ...
  window.Difficulty={score,label,html,levelIndex,bands,band,bandFor};
})();
