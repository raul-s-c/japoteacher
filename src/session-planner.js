(function(){
  ...
  // Collection answers count as evidence, but importing an optional bank must
  // not multiply the general curriculum threshold and relock learned bands.
  function difficultyRoadmap(exercises,attempts,direction){
    ...
    return Object.fromEntries(levels.map(level=>{
      const states=Difficulty.bands.map((band,index)=>{
        const items=exercises.filter(exercise=>exercise.active!==false&&exercise.direction===direction&&exercise.jlpt_level===level&&Difficulty.bandFor(exercise)===index),results=items.map(exercise=>latestByExercise.get(exercise.exercise_id)).filter(Boolean),referenceCount=items.filter(exercise=>!exercise.source_collection).length||items.length,required=Math.min(referenceCount,Math.max(4,Math.ceil(referenceCount*.1))),average=results.length?Math.round(results.reduce((sum,item)=>sum+Number(item.overall_score),0)/results.length):0,acceptable=results.length?results.filter(item=>item.is_acceptable).length/results.length:0;
  ...
  async function replaceExercise(session,exerciseId,reason,settings){
    if(settings.studyPlansVersion&&window.StudyPlans)return StudyPlans.replace(session,exerciseId,reason,settings);
    ...
    const sameSource=candidates.filter(exercise=>(exercise.source_collection||'')===(current.source_collection||'')),sourcePool=sameSource.length?sameSource:candidates,levelCandidates=sourcePool.filter(exercise=>exercise.jlpt_level===current.jlpt_level),pool=levelCandidates.length?levelCandidates:sourcePool;
