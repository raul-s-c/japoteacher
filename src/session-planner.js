(function(){
  ...
  async function buildPlan(profileId,settings,date,options){
    ...
    const changed=!existing||rebalance||['ja_es','es_ja'].some(d=>existing['exercise_ids_'+d+'_json']!==JSON.stringify(plans[d])||existing['voluntary_repeat_ids_'+d+'_json']!==JSON.stringify(repeats[d]))||existing.completed_exercise_ids_json!==JSON.stringify([...completed]);
