(function(){
  ...
  function draw(){
    ...
    $('#selectionStatus').textContent='Se reserva el cupo de nuevas y el resto se completa con repasos de menor a mayor media de las tres últimas notas. Puedes sustituir voluntariamente repasos con media superior al 50 % por nuevas. Tres notas consecutivas de 95 o más retiran la frase del repaso automático, en esta dirección. Al terminar la ronda se repiten, barajadas, las notas inferiores a 50.';
  }
  async function render(settings,session,snap){context={settings,session,snap};plans=await StudyPlans.all(settings.profileId);draw();window.PlanNewReview?.schedule()}
  ...
  window.StudyPlansUI={render,getContext:()=>({context,plans})};
})();
