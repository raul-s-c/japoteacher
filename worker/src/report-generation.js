...
function levelContext(items, exercises) {
  const rows = LEVELS.map(level => {
    const values = items.filter(item => (exercises.get(item.exercise_id)?.jlpt_level || item.jlpt_level) === level), scores = values.map(item => Number(item.overall_score)).filter(Number.isFinite), acceptable = values.filter(item => item.is_acceptable).length, difficulty = values.map(item => Number(exercises.get(item.exercise_id)?.difficulty ?? item.difficulty)).filter(Number.isFinite);
...
export function experienceMetrics(attempts) {
  ...
  const activity = Object.fromEntries(["translation_attempt", "daily_news_answer", "lexical_review"].map(type => { const values = rows.filter(item => (item.study_event_type || "translation_attempt") === type).map(item => Number(item.ranked_xp_delta)); return [type, { count: values.length, net: Math.round(values.reduce((sum, value) => sum + value, 0) * 1000) / 1000 }]; }));
  return { net: Math.round(total * 1000) / 1000, gained: Math.round(Object.values(direction).reduce((sum, item) => sum + item.gained, 0) * 1000) / 1000, lost: Math.round(Object.values(direction).reduce((sum, item) => sum + item.lost, 0) * 1000) / 1000, attempt_count: rows.length, direction, activity, daily: [...daily.values()].sort((left, right) => left.date.localeCompare(right.date)).map(item => Object.fromEntries(Object.entries(item).map(([key, value]) => [key, typeof value === "number" ? Math.round(value * 1000) / 1000 : value]))) };
}
function aggregate(payload, period) {
  ...
  for (const attempt of attempts) for (const error of json(attempt.errors_json)) { const key = `${attempt.direction}|${error.category}|${error.subtype}|${error.source_span}|${error.corrected_span}`, current = errors.get(key) || { direction: attempt.direction, level: exercises.get(attempt.exercise_id)?.jlpt_level || attempt.jlpt_level || "sin nivel", difficulty: exercises.get(attempt.exercise_id)?.difficulty ?? attempt.difficulty ?? null, category: error.category, subtype: error.subtype, source: error.source_span, correction: error.corrected_span, explanation: error.explanation_es, count: 0 }; current.count += 1; errors.set(key, current); }
