...
def main(level):
    ...
    for start in range(0, len(original), 5):
        ...
        corrected_group = [{"level": level, "coverage_slot": previous["coverage_slot"], "review_rounds": previous.get("review_rounds", 0) + rounds, "editorial_quality_version": 2, **by_slot[previous["slot"]]} for previous in group]
        ...
        revalidated += 5
        print(json.dumps({"level": level, "revalidated_this_run": revalidated, "quality_v2_total": sum(item.get("editorial_quality_version", 0) >= 2 for item in original), "total": len(original)}, ensure_ascii=False), flush=True)
    signatures = [EDITORIAL.normalize_japanese(item["japanese"]) for item in original]
    if len(signatures) != len(set(signatures)):
        raise RuntimeError("La revalidación produjo duplicados.")
