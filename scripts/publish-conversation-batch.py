...
def main():
 ...
 with (ROOT/'data/editorial'/f'{STEM}.tsv').open(encoding='utf-8-sig',newline='') as f:items=list(csv.DictReader(f,delimiter='\t'))
 assert len(items)==50
 ...
 for i,item in enumerate(items,1):
  ...
  pair=f'{item["level"]}-CONVERSATION-20260924-{i:03d}';jaid='JAES-'+pair
 ...
 with tempfile.TemporaryDirectory(prefix='japo-conversation-') as folder:
  ...
  builder=load('build-knowledge-map');builder.ROOT=stage;argv=sys.argv;sys.argv=['build-knowledge-map.py','--version',VERSION]
  try:builder.main()
  ...
  for a in audit:
   ...
   # Surface spans as well as base forms cover expressions split by the tokenizer.
   assert any(e['term']==a['target'] for e in evidence) or a['target'] in a['japanese'],('Unproven target',a['id'])
 for n in addition['nodes']:nodes.setdefault(n['id'],n)
 graph['nodes']=list(nodes.values());graph['sentences'].update(addition['sentences']);graph['version']=VERSION
 ...
 (ROOT/'data/editorial'/f'{STEM}-audit.json').write_text(json.dumps(report,ensure_ascii=False,indent=2),encoding='utf-8')
