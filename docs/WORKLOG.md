...
## 2026-09-20 — Lupa de Windows

Aplicación portable Electron para Windows x64: panel siempre encima configurable, burbuja movible, atajos globales, recorte nativo en pantalla del cursor, abrir/arrastrar/pegar imágenes y recepción automática opt-in del portapapeles. Cola de diez capturas, deduplicación del portapapeles, OCR japonés local incluido con modo vertical opcional y visión explícita. X minimiza; salir requiere confirmación. Sin cambios de backend ni gasto OpenAI.
Página dedicada reutiliza src/lens.js, IndexedDB y CloudSync. Conserva análisis, mensajes y candidatos pendientes de revisión con origen lens_desktop. ID de captura estable evita duplicar análisis guardados si se recarga antes de confirmar la cola; texto anterior se limpia al recibir una imagen. Errores dejan la captura preparada. Los datos ya guardados persisten; imágenes pendientes en memoria, avisadas al salir.
169 pruebas Node superadas. Electron real con perfil aislado y backend simulado: importación automática/manual, error y reintento, historial tras recarga, OCR real (学校 reconocido), portapapeles manual/automático sin duplicados, recorte nativo, minimizar y cancelar cierre. Captura visual inspeccionada. Browser plugin not available: Playwright Electron. Sincronización real con la cuenta del usuario entre Windows y Android pendiente de confirmación. Caché web v175. Paquete sin firma digital, descarga ZIP portable.
Comprobación visual final: el renderizador compartido de lupa admite también el campo term en las lecturas y muestra furigana en el texto detectado. Regresión verificada con Electron real; caché final v176. ZIP Windows publicado en la release windows-lupa-v1.0.0 (195.271.413 bytes, SHA-256 en releases/windows).
